  ...
  return (
    ...
    >
      ...
      <div className="card p-4 mb-4 animate-fade-in">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-text-primary">Feel vs Real 추이</h3>
          <button
            type="button"
            ...
            >
              ...
              <circle
                ...
                className={cn(
                  ...
                  stats.feel_accuracy_rate >= 60 ? 'text-status-success' : 'text-status-warning'
                )}
              />
            </svg>
            ...
            <div className="h-12 flex items-end gap-1">
              {stats.feel_accuracy_trend.slice(-7).map((point, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="w-full bg-brand-primary/30 hover:bg-brand-primary/50 rounded-t transition-all duration-300 cursor-pointer"
                      style={{ height: `${height}%` }}
                      title={`${new Date(point.date).toLocaleDateString('ko-KR')}: ${point.accuracy}%`}
                    >
                      <div
                        className="w-full bg-brand-primary rounded-t transition-all duration-300"
                        style={{ height: '100%' }}
                      />
                    </div>
                    <span className="text-[7px] text-text-tertiary mt-1 group-hover:font-semibold">
      ...
      <div className="card p-4 mb-4 animate-fade-in">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-text-primary">주간 목표</h3>
          <button
            type="button"
        ...
        {weeklyGoal ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-text-secondary">목표</span>
              <span className="text-sm font-semibold text-text-primary">{weeklyGoal}회</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={cn(
                'text-xs font-medium',
                stats.current_week_swings >= weeklyGoal ? 'text-status-success' : 'text-status-warning'
              )}>
          ...
          <p className="text-xs text-text-secondary">주간 목표를 설정해보세요!</p>
        )}
      </div>
      ...
      {stats.achievements && stats.achievements.length > 0 && (
        <div className="card p-4 mb-4 animate-fade-in">
          <h3 className="text-sm font-semibold text-text-primary mb-3">달성 배지</h3>
          <div className="grid grid-cols-3 gap-2">
            {stats.achievements.slice(0, 6).map((achievement) => (
              <div
                key={achievement.id}
                className="flex flex-col items-center text-center p-2 rounded-lg bg-gray-50/50 hover:bg-gray-100 transition-colors duration-200"
              >
                <div className="text-2xl mb-1">{achievement.icon}</div>
                <p className="text-[9px] font-medium text-text-primary leading-tight">
                  {achievement.name}
                </p>
          ...
          <div className="flex items-end gap-1 h-24 group">
            {stats.weekly_swings.map((week, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className="w-full bg-brand-primary/20 rounded-t transition-all duration-300 hover:bg-brand-primary/40"
                    style={{ height: `${Math.max(height, 4)}%` }}
                    title={`${week.week}: ${week.count}회`}
                  >
                    <div
                      className="w-full bg-brand-primary rounded-t transition-all duration-300 group-hover/bar:shadow-lg"
                      style={{ height: '100%' }}
                    />
                  </div>
                  <span className="text-[8px] text-text-tertiary group-hover/bar:font-semibold transition-all">
                    {week.week}
                  </span>
                  <span className="text-[7px] text-text-tertiary opacity-0 group-hover/bar:opacity-100 transition-opacity">
                    {week.count}회
                  </span>
                </div>
              );
            ...
            {stats.most_common_errors.slice(0, 5).map((ep, idx) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <span className="text-[10px] bg-gray-100 text-text-tertiary px-1.5 py-0.5 rounded font-mono shrink-0">
                    {ep.code}
                  </span>
                  <span className="text-xs text-text-secondary truncate">{ep.name}</span>
                </div>
                <span className="text-xs font-medium text-text-primary shrink-0 ml-2">{ep.count}회</span>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {stats.recent_reports.map((report) => (
              <button
                ...
                className="card p-3 w-full text-left flex items-center justify-between hover:border-brand-primary/30 transition-all duration-200 transform hover:scale-102"
              >
                ...
                <ChevronIcon className="w-4 h-4 text-text-tertiary shrink-0 ml-2" />
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Goal Modal */}
      {showGoalModal && (
        <div className="fixed inset-0 z-50 bg-black/40 animate-fade-in flex items-center justify-center p-5">
          <div className="card p-6 w-full max-w-sm animate-scale-in">
            <h3 className="text-lg font-semibold text-text-primary mb-4">주간 목표 설정</h3>
            <div className="mb-4">
...
function MiniStat({
  ...
  return (
    <div className="card p-2 text-center hover:shadow-md transition-shadow duration-200">
      <div className="flex items-baseline justify-center gap-1">
        ...
        {change !== undefined && change !== 0 && (
          <p className={cn(
            ...
            change > 0 ? 'text-status-success' : 'text-status-warning'
          )}>
